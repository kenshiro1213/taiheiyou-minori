...
| URL | 内容 |
| --- | --- |
| `#/`            | コース一覧（OUT/INスコアカード形式、各行タップで詳細へ） |
| `#/hole/{1-18}` | ホール詳細1ページ。前後ホールへのページ送り付き。 |
| `#/print`       | A4印刷用シート（OUT 9ホール / IN 9ホール の2ページ、3×3グリッド、書き込み欄付き） |

## 試合当日の使い方

- スマホでサイトを開く（事前に開いておけばオフラインでもキャッシュから表示されやすい）
- コース一覧から該当ホールをタップ → 1ホール1ページ表示
- 各ホール内の「ShotNaviで距離計測 ↗」で当日の任意2点間距離測定が使える
- 危険ポイントは赤帯で強調表示
- 印刷用シート（`#/print`）は事前にA4で印刷すれば書き込み式メモとして持参可能
